...
int my_cd(char **argv, struct exec_struct *e_x)
{
    ...
    if (*arg_cd == '~')
    {
        current_path = find_e_x(e_x, "HOME");
        arg_cd[0] = '.';
    }

    if (*arg_cd == '/')     //gerer les chemins ABS
    {
        assign_var("OLDPWD", current_path, e_x);
        assign_var("PWD", arg_cd, e_x);
    }
    else
        change_pwd_var2(current_path, arg_cd, e_x); // gere les long arg type machin/machin/../.. + met a jour PWD et OLDPWD
